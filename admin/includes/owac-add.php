	...
	public function add_availability_calendar_create_admin_page() {
		...
		if(isset($_GET['edit']) && !empty($_GET['edit'])){ 
			...
			$owac_item = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT * from `{$wpdb->prefix}OWAC_event` 
					WHERE ev_id=%d",
					$id
				)
			);
			...
			<form action="" method="post" onsubmit="return validateForm()" enctype="multipart/form-data">
				...
				<p class="submit">
					<?php if(isset($_GET['edit']) && !empty($_GET['edit'])){ ?>
						<input type="hidden" name="ev_id" id="ev_id" value="<?php esc_html_e( intval($this->add_availability_calendar->ev_id), 'availability-calendar' ); ?>">
					<?php } ?>
	...
	public function cat_id_callback() {
		?> <select name="add_availability_calendar[cat_id]" id="cat_id" required="required">
			...
			<?php 	
					...
					$total_pages_sql = $wpdb->get_results("SELECT cat_id,cat_name FROM $table_prefix WHERE 1 AND `flag`='0'");
			...
			?>
				...
				<option value="<?php esc_html_e( $row->cat_id, 'availability-calendar' ); ?>" <?php esc_html_e( $selected, 'availability-calendar' ); ?>><?php esc_html_e( $row->cat_name, 'availability-calendar' ); ?></option>
			<?php } ?>
		</select> <?php
	}
