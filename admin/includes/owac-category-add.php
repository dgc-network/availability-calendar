...
?>
			...
			<form action="" method="post" onsubmit="return validateForm()" enctype="multipart/form-data">
				...
				<p class="submit">
					<?php if(isset($_GET['edit']) && !empty($_GET['edit'])){ ?>
						<input type="hidden" name="cat_id" id="cat_id" value="<?php esc_html_e( intval($this->add_category_options->cat_id), 'availability-calendar' ); ?>">
					<?php } ?>
	...
	public function add_category_sanitize($input) {
		...
		if ( isset( $input['cat_name'] ) ) {
			$sanitary_values['cat_name'] = sanitize_text_field( $input['cat_name'] );
		}
	...
	public function cat_name_callback() {
		printf(
			'<input class="regular-text" type="text" name="add_category[cat_name]" id="cat_name" value="%s" required>',
			isset( $this->add_category_options->cat_name ) ? esc_attr( $this->add_category_options->cat_name) : ''
		);
	}
