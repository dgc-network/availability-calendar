...
class OWAC_Settings {
	...
	public function settings_create_admin_page() {
		...
		if(empty($this->settings_options['old_date_fade_out'])){
			$this->settings_options['old_date_fade_out'] = esc_attr('no');
		}
		if(empty($this->settings_options['old_date_fade_out_color'])){
			$this->settings_options['old_date_fade_out_color'] = sanitize_hex_color_no_hash('DDDDDD');
		}
?>
		<div class="wrap owac">
			<p></p>
	...
	public function settings_page_init() {
		...
		add_settings_field(
			'old_date_fade_out', // id
			'Fade out past dates', // title
			array( $this, 'old_date_fade_out_callback' ), // callback
			'settings-admin', // page
			'settings_setting_section' // section
		);
		
		add_settings_field(
			'old_date_fade_out_color', // id
			'Fade colour', // title
			array( $this, 'old_date_fade_out_color_callback' ), // callback
			'settings-admin', // page
			'settings_setting_section' // section
		);
		...
		add_settings_field(
			'display_start_day', // id
			'First Day Of The Week:', // title
			array( $this, 'display_start_day_callback' ), // callback
			'settings-admin', // page
			'settings_setting_section' // section
		);
	...
	public function settings_sanitize($input) {
		...
		if ( isset( $input['header_add_text'] ) ) {
			$sanitary_values['header_add_text'] = sanitize_textarea_field( $input['header_add_text'] );
		}
		...
		if ( isset( $input['old_date_fade_out'] ) ) {
			$sanitary_values['old_date_fade_out'] = sanitize_text_field( $input['old_date_fade_out']);
		}
		
		if ( isset( $input['old_date_fade_out_color'] ) ) {
			$sanitary_values['old_date_fade_out_color'] = sanitize_text_field($input['old_date_fade_out_color']);
		}
		...
		if ( isset( $input['display_start_day'] ) ) {
			$sanitary_values['display_start_day'] = sanitize_text_field( $input['display_start_day']);
		}
	...
	public function old_date_fade_out_callback() {
		?> <fieldset><?php $checked = ( isset( $this->settings_options['old_date_fade_out'] ) && $this->settings_options['old_date_fade_out'] === 'yes' ) ? 'checked' : '' ; ?>
		<label for="old_date_fade_out"><input type="radio" name="OWAC_settings_option[old_date_fade_out]" id="old_date_fade_out" value="yes" <?php esc_html_e( $checked, 'availability-calendar' ); ?>> <?php esc_html_e( 'Yes', 'availability-calendar' ); ?></label><br>
		<?php $checked = ( isset( $this->settings_options['old_date_fade_out'] ) && $this->settings_options['old_date_fade_out'] === 'no' ) ? 'checked' : '' ; ?>
		<label for="old_date_fade_out"><input type="radio" name="OWAC_settings_option[old_date_fade_out]" id="old_date_fade_out" value="no" <?php esc_html_e( $checked, 'availability-calendar' ); ?>> <?php esc_html_e( 'No', 'availability-calendar' ); ?></label></fieldset> <?php
	}
	
	public function old_date_fade_out_color_callback() {
		printf(
			'<input class="regular-text jscolor" type="text" name="OWAC_settings_option[old_date_fade_out_color]" id="old_date_fade_out_color" value="%s">',
			isset( $this->settings_options['old_date_fade_out_color'] ) ? esc_attr( $this->settings_options['old_date_fade_out_color']) : ''
		);
	}
	...
	public function display_start_day_callback() {
		?> <select name="OWAC_settings_option[display_start_day]" id="display_start_day">
			<?php $selected = (isset( $this->settings_options['display_start_day'] ) && $this->settings_options['display_start_day'] === '1') ? 'selected' : '' ; ?>
			<option value="1" <?php esc_html_e( $selected, 'availability-calendar' ); ?>><?php esc_html_e( 'Monday', 'availability-calendar' ); ?></option>
			<?php $selected = (isset( $this->settings_options['display_start_day'] ) && $this->settings_options['display_start_day'] === '2') ? 'selected' : '' ; ?>
			<option value="2" <?php esc_html_e( $selected, 'availability-calendar' ); ?>><?php esc_html_e( 'Tuesday', 'availability-calendar' ); ?></option>
			<?php $selected = (isset( $this->settings_options['display_start_day'] ) && $this->settings_options['display_start_day'] === '3') ? 'selected' : '' ; ?>
			<option value="3" <?php esc_html_e( $selected, 'availability-calendar' ); ?>><?php esc_html_e( 'Wednesday', 'availability-calendar' ); ?></option>
			<?php $selected = (isset( $this->settings_options['display_start_day'] ) && $this->settings_options['display_start_day'] === '4') ? 'selected' : '' ; ?>
			<option value="4" <?php esc_html_e( $selected, 'availability-calendar' ); ?>><?php esc_html_e( 'Thursday', 'availability-calendar' ); ?></option>
			<?php $selected = (isset( $this->settings_options['display_start_day'] ) && $this->settings_options['display_start_day'] === '5') ? 'selected' : '' ; ?>
			<option value="5" <?php esc_html_e( $selected, 'availability-calendar' ); ?>><?php esc_html_e( 'Friday', 'availability-calendar' ); ?></option>
			<?php $selected = (isset( $this->settings_options['display_start_day'] ) && $this->settings_options['display_start_day'] === '6') ? 'selected' : '' ; ?>
			<option value="6" <?php esc_html_e( $selected, 'availability-calendar' ); ?>><?php esc_html_e( 'Saturday', 'availability-calendar' ); ?></option>
			<?php $selected = (isset( $this->settings_options['display_start_day'] ) && $this->settings_options['display_start_day'] === '7') ? 'selected' : '' ; ?>
			<option value="7" <?php esc_html_e( $selected, 'availability-calendar' ); ?>><?php esc_html_e( 'Sunday', 'availability-calendar' ); ?></option>
		</select> <?php
	}
